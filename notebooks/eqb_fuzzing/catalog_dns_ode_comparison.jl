...
const NC_FILE_RE = r"^([A-G])_J(\d+)K(\d+)L(\d+)_ubest\.nc$"
...
function parse_local_nc_filename(name::AbstractString)
    m = match(NC_FILE_RE, name)
    m === nothing && return nothing
    return (
        group = m.captures[1],
...
function find_matching_local_nc(sol_dir::AbstractString, best_asc, fallback_path::AbstractString="")
    exact = joinpath(sol_dir, "$(best_asc.group)_J$(best_asc.J)K$(best_asc.K)L$(best_asc.L)_ubest.nc")
    isfile(exact) && return exact

    best = nothing
    for name in readdir(sol_dir)
        endswith(name, ".nc") || continue
        parsed = parse_local_nc_filename(name)
        parsed === nothing && continue
        cand = merge(parsed, (nc_path = joinpath(sol_dir, name),))
        better = if best === nothing
            true
        elseif cand.group == best_asc.group && best.group != best_asc.group
            true
        elseif cand.group == best_asc.group && best.group == best_asc.group && cand.jkl > best.jkl
            true
        elseif best.group != best_asc.group && cand.jkl > best.jkl
            true
        else
            false
        end
    ...
    if best !== nothing
        return best.nc_path
    elseif !isempty(fallback_path) && isfile(fallback_path)
...
function render_comparison(row, solutions_dir::AbstractString, out_dir::AbstractString, model_cache; args=Dict{String,String}())
    ...
    dns_path = find_matching_local_nc(sol_dir, best, get(row, :representative_ubest, ""))
    dns_path === nothing && error("No local or representative ubest.nc found for $pid")
    ...
    fig = Base.invokelatest(
        CloudAtlas.flowfield3d_comparison,
        model,
        x,
        dns_path;
        baseflow=baseflow,
    ...
    println("Saved $out_png   (ode $(best.source)_$(best.group)_J$(best.J)K$(best.K)L$(best.L), dns $(basename(dns_path)))")
    return (
        physical_id = pid,
...
function write_manifest(path::AbstractString, rows)
    open(path, "w") do io
        println(io, "physical_id,case,Re,Lx,Lz,group,source,J,K,L,asc_path,dns_path,out_png")
        for r in rows
            println(io,
                string(r.physical_id, ",", r.case, ",", r.Re, ",", r.Lx, ",", r.Lz, ",",
                       r.group, ",", r.source, ",", r.J, ",", r.K, ",", r.L, ",",
                       r.asc_path, ",", r.dns_path, ",", r.out_png))
        end
    end
end
...
function main(args=ARGS)
    ...
    out_root = get(parsed, "out-dir", joinpath(DEFAULT_CATALOG_DIR, "dns_ode_comparison_" * Dates.format(now(), "yyyymmdd_HHMMSS")))
    ...
    if dry_run
        for row in eachrow(selected)
            ...
            dns_path = find_matching_local_nc(sol_dir, best, get(row, :representative_ubest, ""))
            dns_label = dns_path === nothing ? "MISSING_DNS" : basename(dns_path)
            println("  $pid   $(row.case)   ode=$(best.source)_$(best.group)_J$(best.J)K$(best.K)L$(best.L)   dns=$(dns_label)")
        end
        return
    end
    ...
    for row in eachrow(selected)
        push!(manifest_rows, render_comparison(row, solutions_dir, out_root, model_cache; args=parsed))
    end
