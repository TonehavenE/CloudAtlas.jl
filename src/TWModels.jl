...
Check if symmetry group H contains a shift symmetry in the given direction (:x or :z).
If it does, we don't need a phase constraint in that direction.
"""
function has_shift_symmetry(H::Vector{Symmetry}, direction::Symbol)
    for σ in H
        if direction == :x && σ.ax != 0//1 && σ.ax != 1//1
            return true  # Non-trivial x-shift exists
        elseif direction == :z && σ.az != 0//1 && σ.az != 1//1
            return true  # Non-trivial z-shift exists
        end
    end
    return false
end
...
"""
function TWModel(α::T, γ::T, J::Int, K::Int, L::Int, H::Vector{Symmetry}; 
                 normalize=false, tol_phase = 1e-12) where T<:Real
    ...
    # Determine phase constraints from symmetry group
    ...
    TWModel(α, γ, H, ijkl, Ψ, Ψshear, B, A1, A2, Cx, Cz, N, Bfact, 
            m, keep_cx, keep_cz, f, Df, g, Dg)
end
